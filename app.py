...
<div class="page">
{% if s.get('system_alerts') %}
<div style="margin:-4px 0 16px;padding:10px 16px;background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.4);border-radius:10px;display:flex;align-items:flex-start;gap:10px">
  <span style="font-size:16px;line-height:1.4">🚨</span>
  <div>
    <div style="font-size:11px;font-weight:700;color:#F87171;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px">System Alert</div>
    {% for a in s.system_alerts %}<div style="font-size:11px;color:#FCA5A5;line-height:1.5">{{ a.ts }} — {{ a.msg }}</div>{% endfor %}
  </div>
</div>
{% endif %}
